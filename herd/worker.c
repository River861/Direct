...
void *run_worker(void *arg) {
  ...
  for (i = 0; i < num_server_ports; i++) {
    ...
    cb[i] = hrd_ctrl_blk_init(
        ...
        NUM_UD_QPS, 4096, -1, 0, 0); /* num_dgram_qps, dgram_buf_size, key */
    ...
    // setup remote memory
    {
      ...
      mem_cb[i] = hrd_ctrl_blk_init(
          ...
          NULL, 2048, -1, 0, 0, -1, 0, 0); /* #dgram qps, buf size, shm key */
      ...
      for (j = 0; j < NUM_MEMORY; j++) {
        ...
        for (per_mr = 0; per_mr <= HERD_NUM_KEYS; per_mr++) {  // TODO 更改mr_name
