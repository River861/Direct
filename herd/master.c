...
void *run_master(void *arg) {
  ...
  hrd_red_printf(
      "[CAUTION] with %d memory, sizeof mica_op %d\n"
      "Running HERD master with num_server_ports = %d "
      "base_port_index = %d, RR_SIZE = %d MB, RR use fraction = %.2f\n",
      master_params.num_memory, sizeof(struct mica_op), num_server_ports,
      base_port_index, RR_SIZE / M_1,
  ...
  for (i = 0; i < NUM_CLIENT_NODE * NUM_CLIENTS; i++) {
